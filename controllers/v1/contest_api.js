...
module.exports.getTest = async function(req,res){
    ...
    try{
        ...
        const registeredCandidates = await ContestRegistration.find({});
        if( (req.user.id != contest.createdBy) && !(current_status == 'TEST_STARTED' || current_status == 'TEST_COMPLETE')){
            console.log("VVVVV: " , current_status);
